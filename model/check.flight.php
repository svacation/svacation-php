...
class Check_Flight{
    ...
    public function check_flight(){
        ...
        if ($result->num_rows > 0){
            ...
            if($check_retrieve['flight'] <= 0){
                echo "<script type=\"text/javascript\">alert('您的接机送机次数不够，请联系管理员！ ');window.history.back();</script>"; 
                exit();
            }
    ...
    public function remainNum(){
        ...
        if ($result->num_rows > 0){
            $check_retrieve = $result->fetch_assoc();
            echo "<span style=\"font-size:20px;color:red;\">剩余：". $check_retrieve['flight'] . " 次</span>";
